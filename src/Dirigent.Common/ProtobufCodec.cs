...
namespace Dirigent.Net
{
    public class ProtoBufCodec
    {
        ...
        System.IO.MemoryStream _outBodyStream = new System.IO.MemoryStream(1000);
        Dictionary<uint, System.Type> _typeMap;

        public ProtoBufCodec( Dictionary<uint, System.Type> typeMap )
        {
            _typeMap = typeMap;
            msgCodec.MessageReceived = OnMsgReceived;
        }
		...
		void OnMsgReceived( MessageCodec.Header hdr, byte[] data, long offset, long size )
		{
            ...
            if( t != null )
            {
                ...
                MessageReceived( hdr.MsgCode, instance );
            }
		}
        ...
        {
            msgCodec.ReceivedMessagePart( buffer, offset, size );
        }
    }
