...
DC.ExecuteCommand(""cm_ExecuteCmdLine"", ""<load {vpConfFilePath.Replace("\\", "\\\\")}"")
		...
		void AddToVpConf( StringBuilder sb, VfsNodeDef node, string parentPath )
		{
			foreach (var child in node.Children)
			{
				...
				var virtPath = $"{parentPath}\\{GetNodeName(child)}";
		...
		/// <summary>
		/// Name of the node within the virtual panel.
		/// Virtual folders have no physical path, so their title is used.
		/// </summary>
		static string GetNodeName( VfsNodeDef node )
		{
			var name = Path.GetFileName( node.Path ?? string.Empty );
			if( string.IsNullOrEmpty( name ) ) name = node.Title;
			if( string.IsNullOrEmpty( name ) ) name = node.Id;
			if( string.IsNullOrEmpty( name ) ) name = node.IsContainer ? "folder" : "file";

			// a title like "Logs/Recent" denotes a submenu path - take just the last segment of it
			return name.Split( new char[] {'/','\\'}, StringSplitOptions.RemoveEmptyEntries ).LastOrDefault() ?? name;
		}

		void DeleteOldFiles( string folder )
		{
			// our own temp folder holds nothing but the files generated by the previous runs
			var tempFiles = Directory.GetFiles( folder, "*" );
